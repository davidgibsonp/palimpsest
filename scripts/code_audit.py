...
def main():
    ...
    for file_path in python_files:
        ...
        print(f"{str(rel_path):45} {audit['total_lines']:4d} lines  {audit['classes']:2d} classes  {audit['functions']:2d} funcs")
    
    print("-" * 60)
    print(f"{'TOTAL':45} {total_lines:4d} lines  ({total_code_lines} code, {total_comment_lines} comments)")
    print()
    ...
    poorly_documented = [audit for audit in audits if audit["comment_ratio"] < 0.15 and audit["total_lines"] > 20]
    ...
    for module, stats in sorted(module_stats.items()):
        print(f"   {module:15} - {stats['classes']} classes, {stats['functions']} functions, {stats['lines']} lines")
    ...
    if long_functions:
        recommendations.append("Consider breaking down long functions into smaller, focused functions")
    ...
    if total_classes > 15:
        recommendations.append("Consider if all classes are necessary - might be over-engineered")
    
    if total_lines > 2000:
        recommendations.append("Codebase is getting large - consider modularization strategies")
    
    # Migration framework complexity check
    migration_files = [audit for audit in audits if "migration" in audit["path"].lower()]
    if migration_files:
        migration_lines = sum(audit["total_lines"] for audit in migration_files)
        if migration_lines > 300:
            recommendations.append("Migration framework might be over-engineered for v0.1.0")
    ...
    if issues == 0:
        ...
        print("🟡 CAUTION: Minor issues found, but acceptable for continued development")
    else:
        print("🔴 STOP: Multiple quality issues found - recommend refactoring before continuing")
