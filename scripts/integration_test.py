...
def test_full_workflow():
    ...
    for i, trace_id in enumerate(trace_ids, 1):
        ...
        print(
            f"📖 Loaded trace {i}: {loaded_trace.problem_statement[:60]}... ({load_time:.1f}ms)"
        )
        print(f"   ✅ Schema version: {loaded_trace.schema_version}")
        print(f"   ✅ Steps: {len(loaded_trace.execution_steps)}")
        print(
            f"   ✅ Domain: {loaded_trace.domain}, Complexity: {loaded_trace.complexity}"
        )
        print()
...
def test_migration_compatibility():
    ...
    print(f"   Schema version: {loaded_trace.schema_version}")
    print(
        f"   Migration metadata: {loaded_trace.context.environment.get('migrated_from') if loaded_trace.context.environment else 'none'}"
    )
