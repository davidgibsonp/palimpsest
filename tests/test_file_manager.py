...
def test_get_trace_stats(file_manager, sample_trace):
    """Test getting trace statistics."""
    # Empty stats
    stats = file_manager.get_trace_stats()
    ...
    assert stats["storage_size_bytes"] > 0
    assert stats["oldest_trace"] in trace_ids
    assert stats["newest_trace"] in trace_ids
    assert "storage_directory" in stats


def test_cleanup_corrupted_files(file_manager, temp_dir):
    """Test cleanup of corrupted JSON files."""
    traces_dir = temp_dir / ".palimpsest" / "traces"
    
    # Create a valid trace
    sample_trace = ExecutionTrace(
        problem_statement="Valid trace",
        outcome="Success",
        execution_steps=[
            ExecutionStep(step_number=1, action="test", content="valid")
        ]
    )
    trace_id = file_manager.save_trace(sample_trace)
    
    # Create a corrupted JSON file
    corrupted_file = traces_dir / "corrupted.json"
    with open(corrupted_file, 'w') as f:
        f.write("{ invalid json")
    
    # Create an invalid trace file (valid JSON but invalid trace)
    invalid_trace_file = traces_dir / "invalid_trace.json"
    with open(invalid_trace_file, 'w') as f:
        json.dump({"not": "a_valid_trace"}, f)
    
    # Cleanup should remove corrupted files
    removed = file_manager.cleanup_corrupted_files()
    
    # Should remove both corrupted files
    assert len(removed) == 2
    assert "corrupted.json" in removed
    assert "invalid_trace.json" in removed
    
    # Valid trace should remain
    assert file_manager.trace_exists(trace_id)
    assert not corrupted_file.exists()
    assert not invalid_trace_file.exists()
